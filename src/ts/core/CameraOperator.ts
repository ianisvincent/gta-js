...
export class CameraOperator implements IInputReceiver, IUpdatable {
    ...
    public update(timeScale: number): void {
        if (this.followMode === true) {
            // Constrain camera position Y = min is target.y, max is infinity
            // In this world, Y is up and down (?)
        ...
        if (this.aimingMode && this.targetedCharacter.isAiming && this.targetedCharacter.hasWeapon) {
    ...
    public setAimingCameraMode(): void {
        ...
        this.targetedCharacter.setOrientation(cameraDirection, true);
    }
